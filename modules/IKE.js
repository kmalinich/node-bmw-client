...
module.exports = {
  ...
  decode_aux_heat_led       : (data)                  => { decode_aux_heat_led(data);       },
  decode_ignition_status    : (data)                  => { decode_ignition_status(data);    },
  decode_odometer           : (data)                  => { decode_odometer(data);           },
  ...
  obc_clock                 : ()                      => { obc_clock();                     },
  obc_data                  : (action, value, target) => { obc_data(action, value, target); },
  obc_refresh               : ()                      => { obc_refresh();                   },
  parse_out                 : (data)                  => { parse_out(data);                 },
  request                   : (value)                 => { request(value);                  },
  text                      : (msg, to, dir, turn)    => { text(msg, to, dir, turn);        },
  text_override             : (msg, to)               => { text_override(msg, to);          },
  text_urgent               : (msg)                   => { text_urgent(msg);                },
  text_urgent_off           : ()                      => { text_urgent_off();               },
  text_warning              : (msg, to)               => { text_warning(msg, to);           },
};
