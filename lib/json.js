...
function reset(reset_cb = null) {
	modules_reset(() => { // Set modules as not ready
		status_reset(() => { // Reset some variables
...
async function config_read(config_read_cb = null) {
	...
	try {
		...
		await config_write(config_read_cb);
		return false;
	}
...
async function status_read(status_read_cb = null) {
	...
	try {
		...
		await status_write(status_read_cb);
		return false;
	}
...
function init_listeners() {
	...
	power.on('active', (power_state) => {
		switch (power_state) {
			case false : {
				// Set modules as not ready
				if (config.json.reset_on_poweroff) modules_reset();

				// Write JSON config and status files
				if (config.json.write_on_poweroff) write();
			...
			case true : {
				// Write JSON config and status files
				if (config.json.write_on_run) write();
			}
		}
	});
	...
	update.on('status.vehicle.clutch_count', () => {
		// Write JSON config and status files
		if (config.json.write_on_gear_change) write();
	});

	// Write JSON data on manual trans gear change event, if configured
	update.on('status.egs.gear', () => {
		// Write JSON config and status files
		if (config.json.write_on_gear_change) write();
	});
	...
	update.on('status.server.connected', (data) => {
		if (data.new === false) status_reset_basic();
	});
