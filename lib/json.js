...
function config_write(config_write_callback = null) {
	...
	if (typeof config.system === 'undefined') {
		...
		if (typeof config_write_callback !== 'function') return;

		setTimeout(() => {
			process.nextTick(config_write_callback);
	...
	if (typeof config_write_callback === 'function') {
		setTimeout(() => {
			process.nextTick(config_write_callback);
			config_write_callback = undefined;
		}, 250);
...
function status_write(status_write_callback = null) {
	...
	if (typeof status.system === 'undefined') {
		...
		if (typeof status_write_callback !== 'function') return;

		setTimeout(() => {
			process.nextTick(status_write_callback);
	...
	if (typeof status_write_callback === 'function') {
		setTimeout(() => {
			process.nextTick(status_write_callback);
			status_write_callback = undefined;
		}, 250);
