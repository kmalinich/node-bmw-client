...
module.exports = {
	...
	command : (action) => {
		if (config.media.hdmi === true && HDMI.hdmi_client !== null && status.hdmi.client_ready === true) {
			switch (action) {
				case 'powerstatus':
					log.msg({
						src : 'HDMI',
					...
					HDMI.hdmi_client.sendCommand(0xF0, CEC.Opcode.GIVE_DEVICE_POWER_STATUS);
					break;
				...
				case 'poweron':
					...
					HDMI.hdmi_client.sendCommand(0xF0, CEC.Opcode.IMAGE_VIEW_ON);
					// Get status
					setTimeout(() => {
						HDMI.command('powerstatus');
					}, 5000);
				...
				case 'poweroff':
					...
					HDMI.hdmi_client.sendCommand(0xF0, CEC.Opcode.STANDBY);
					// Get status
					setTimeout(() => {
						HDMI.command('powerstatus');
					}, 5000);
