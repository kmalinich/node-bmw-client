...
module.exports = {
	...
	startup : (startup_callback) => {
		...
		socket.io.on('connect', () => {
			kodi.notify(module_name, 'connected to server');
			log.msg({
				src : module_name,
		...
		socket.io.on('connect_error', (error) => {
			...
			kodi.notify(module_name, 'connect error: '+error.description);
			log.msg({
				src : module_name,
		...
		socket.io.on('reconnect', (number) => {
			...
			var log_msg = 'Reconnected after '+number+' tries';
			kodi.notify(module_name, log_msg);
			log.msg({
				src : module_name,
				msg : log_msg,
			});
		...
		socket.io.on('disconnect', () => {
			...
			kodi.notify(module_name, 'disconnected from server');
			log.msg({
				src : module_name,
