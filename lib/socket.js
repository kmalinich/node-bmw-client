...
module.exports = {
	...
	startup : (startup_callback) => {
		...
		socket.io.on('data-receive', (data) => {
			switch (data.bus) {
				case 'canbus' : data_handler.check_data_canbus(data);
				default       : data_handler.check_data(data);
			}
		});
	},
	...
	// Send vehicle CANBUS data to WebSocket
	data_send_canbus : (data) => {
		if (config.server.bus === true) {
			data.host = os.hostname().split('.')[0];
			socket.check(() => { socket.io.emit('data-send-canbus', data); });
		}
	},
