...
// Socket connection statistics output setInterval container
const statsInterval = {};
...
class socket extends EventEmitter {
	...
	async reconnect(intf, overrideReconnectingCheck = false) {
		...
		if (overrideReconnectingCheck !== true && status.server.sockets[intf].reconnecting !== false) return;
		...
		// Try again 3-6 seconds later
		setTimeout(() => { this.reconnect(intf, true); }, Math.floor(Math.random() * 3000) + 3000);
	} // async reconnect(intf, overrideReconnectingCheck)
	...
	async init() {
		...
		for (const configuredIntf in config.intf) {
			// Skip if this interface is disabled
			if (config.intf[configuredIntf].enabled !== true) continue;

			update.status(`server.sockets.${configuredIntf}.connected`,    false);
			update.status(`server.sockets.${configuredIntf}.connecting`,   false);
			update.status(`server.sockets.${configuredIntf}.reconnecting`, false);

			log.lib(`Initializing '${configuredIntf}'`);
			...
			this.intf[configuredIntf] = new jsoc(new net.Socket(socket_options));

			// Enable keepalive
			await this.intf[configuredIntf]._socket.setKeepAlive(true);

			// Disable Nagle algorithm
			await this.intf[configuredIntf]._socket.setNoDelay(true);


			// Periodically output socket stats
			if (typeof statsInterval[configuredIntf] === 'undefined') {
				statsInterval[configuredIntf] = null;
			}

			clearInterval(statsInterval[configuredIntf]);
			statsInterval[configuredIntf] = null;

			statsInterval[configuredIntf] = setInterval(() => {
				const statsJSON = JSON.stringify({
					bufferSize   : this.intf[configuredIntf]._socket.bufferSize,
					bytesRead    : this.intf[configuredIntf]._socket.bytesRead,
					bytesWritten : this.intf[configuredIntf]._socket.bytesWritten,
				});

				log.lib(`stats ${configuredIntf} ${statsJSON}`);
			}, 300000);

			await this.init_intf_listeners(configuredIntf);

			// Initialize interfaces
			await this.init_intf(configuredIntf);

			log.lib(`Initialized '${configuredIntf}'`);
		} // for (const configuredIntf in config.intf)
