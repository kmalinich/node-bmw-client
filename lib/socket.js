...
module.exports = {
	...
	startup : (startup_callback) => {
		...
		socket.io.on('con-button', (data) => {
			if (data.action == 'press') {
				console.log('con-button', data.button);
				kodi.input(data.button);
			}
		});

		socket.io.on('con-rotate', (data) => {
			console.log('con-rotate', data.rotation.direction);
				kodi.input(data.rotation.direction);
		});
	},
