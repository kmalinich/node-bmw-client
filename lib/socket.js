...
function on_data(data) {
	...
	// Loadbalance messages across workers
	// if (cluster.isMaster) {
	// 	// log.msg({
	// 	// 	src : module_name,
	// 	// 	msg : 'Master sending '+data.event+' to worker',
	// 	// });

	// 	let keys = Object.keys(cluster.workers)
	// 	cluster.workers[keys[keys.length*Math.random()<<0]].send(data);
	// 	return;
	// }
...
// 		log.msg({
// 			src : module_name,
// 			msg : 'Worker sending '+event+' to master',
// 		});
// 
// 		process.send({
// 			event : event,
...
function init(init_callback = null) {
	// Don't allow workers to connect directly
	// if (cluster.isWorker) {
	// 	// Set up worker reciever
