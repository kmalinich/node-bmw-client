...
function dst_detect(dst) {
	// No dst? Must be CANBUS
	if (typeof dst === 'undefined' || dst === null) return 'canbus';
...
function parse_canbus(data) {
	...
	if (modules[data.src.upper] != null) modules[data.src.upper].parse_out(data);
}
...
module.exports = {
	...
	send : (data) => {
		socket.bus_tx(dst_detect(data.dst), data);
	},
