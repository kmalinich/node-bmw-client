...
function bus_detect(data) {
	...
	// TODO handle this properly, since if undefined it will all go to the iDrive knob on can1
...
function parse_canbus(data) {
	...
	// Workaround for zeroMQ AMP encoding
...
function parse_low(data) {
	...
	// Make a copy for emulated module parse_in functions
	let data_in = data;
	...
	switch (data.msg[0]) {
		...
		case 0x02: // Broadcast: Device status
			...
			update.status(data.src.lower + '.ready', true);

			if (data.msg[1] === 0x01) {
				// Set device status reset var
				update.status(data.src.lower + '.reset', false);
		...
		case 0x23: { // Update: Display text
			...
			// data.msg[2] - flags
			let flags = [];
			if (bitmask.test(data.msg[2], 0x01)) { flags.push('Bit0');      }
			if (bitmask.test(data.msg[2], 0x02)) { flags.push('Bit1');      }
			if (bitmask.test(data.msg[2], 0x04)) { flags.push('Bit2');      }
		...
		case 0x32: { // Broadcast: Volume control
			...
			if (status.kodi.volume.level === null) update.status('kodi.volume.level', config.media.kodi.default_volume);
	...
	if (config.emulate[data.dst.lower] === true) modules[data.dst.upper].parse_in(data_in);
...
module.exports = {
	send : (data) => {
		...
		let dst_bus = bus_detect(data);
		socket.bus_tx(dst_bus, data);
	},
