...
const modules = {
	...
	CSU  : CSU,  CVM  : CVM,  DIA  : DIA,  DME  : DME,  DMEK : DMEK,
	...
	LCM  : LCM,  LWS  : LWS,  MFL  : MFL,  MID  : MID,
	...
	switch (data.msg[0]) {
		...
		case 0x23: { // Update: Display text
			...
			switch (data.msg[1]) {
				...
				case 0x50 : layout = 'cluster';      return;
		...
		case 0x32: { // Broadcast: Volume control
			...
			if (config.media.kodi.enable && config.media.kodi.mfl_volume) {
				let volume_target;

				if (status.kodi.volume.level === null) status.kodi.volume.level = 25;
...
module.exports = {
	send    : (data) => {
		...
		let dst_bus = dst_detect(data);
		console.log('sending, dst data: %s, dst bus: %s', data.dst, dst_bus);
		if (data.dst == 'bus-data') {
			console.log(data);
		}
