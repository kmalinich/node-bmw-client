...
function parse_canbus_kcd(data) {
	...
	switch (data.topic) {
		case 'dme.voltage' : update_quiet = true; break;

		// case 'engine.fueling.active'    :
		// case 'engine.fueling.cut'       :
		case 'engine.fueling.full_load' : update_quiet = true; break;

		case 'engine.ac.clutch'   :
		case 'engine.ac.request'  :
		case 'engine.ac.torque'   :
		case 'engine.dsc_ok'      :
	...
	switch (data.topic) {
		...
		case 'engine.fueling.active'    :
		case 'engine.fueling.cut'       :
		case 'engine.fueling.full_load' : data.msg = Boolean(data.msg); break;
...
function parse_canbus(data) {
	// For data (pre) parsed by bmwi/lib/intf-can.js
	if (data.type === 'parsed-canbus') return parse_canbus_kcd(data);
...
function parse_isp2(data) {
	...
	for (const key in data.data) {
		update.status(`${data.topic}.${key}`, data.data[key]);
	}
}
...
function parse_low(data) {
	...
	switch (data.msg[0]) {
		...
		case 0x73 : { // Request: Immobilizer status
			data.command = 'req';
	...
	// Parse sent data to emulated modules, if parse_in() function exists
	// TODO: Only check once for parse_in function, at app startup
	// if (typeof modules[data.dst.upper].parse_in === 'function')
...
function receive(data) {
	...
	switch (data.bus) {
		...
		case 'isp2' : parse_isp2(data); break;
...
function send(data) {
	...
	// If not in loopback mode, send the data to the corresponding bmwi interface
	if (config.loopback === false) return socket.bus_tx(data);


	// If in loopback mode, modify it to match incoming data and "receive" it
