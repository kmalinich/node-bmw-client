...
async function command(action) {
	...
	switch (action) {
		case 'Connect'    : TEL.led({ solid_red : true, flash_green : true }); break;
		case 'Disconnect' : TEL.led({ solid_green : true, flash_red : true });
	}
...
function interface_properties_changed_device(property, data1, bt_interface) {
	...
	switch (key) {
		case 'connected' : { // Just update TEL LED and return if the key name is 'connected' (connected status)
			...
			if (status.bluetooth.device.connected === false) update.status('bluetooth.device.status', 'paused', false);
...
function interface_properties_changed_player(property, data1, bt_interface) {
	...
	if (data1[0][0].toLowerCase() !== 'position') {
		log.lib('Player \'' + bt_interface + '\' property \'' + property + '\' (\'' + data1[0][0].toLowerCase() + '\') changed');
	}
	...
	switch (typeof data1[0][1][1][0]) {
		...
		default : {
			...
			if (config.media.bluetooth.text.ike === true) {
				...
				// Lowercase 'Feat' to 'feat' and split it
				const strings = {
					artist : status.bluetooth.media.artist.replace(/feat/gi, 'feat').split(' (feat. ')[0],
					title  : status.bluetooth.media.title.replace(/feat/gi, 'feat').split(' (feat. ')[0],
				};
	...
	bluetooth.system_dbus.getObject(bluetooth.paths.bluez.main, '/', (error, objects) => {
		...
		objects.as(bluetooth.paths.dbus.obj_man).GetManagedObjects(async (error_obj, return_obj) => {
			...
			switch (parse.name) {
				case 'public'      :
				case '/NowPlaying' : {
					log.lib('Device \'' + parse.name + '\' (' + parse.path + ') is invalid, incrementing device counter (' + bluetooth.device_num + ')');
					bluetooth.device_num++;
					return;
				}
			}
