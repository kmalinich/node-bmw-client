...
async function command(action) {
	...
	log.lib(`Attempting to send '${action}' command to device '${bluetooth.interfaces.device}'`);

	if (bluetooth.interfaces.device === null) {
	...
	log.lib(`Sending '${member}' command to device '${status.bluetooth.device.name}' ('${bluetooth.interfaces.device}')`);
	...
	switch (member) {
		...
		case 'Disconnect' : {
			const dbus_main = {
				...
				path        : bluetooth.interfaces.device,
				...dbus_base,
			};
		...
		case 'Previous' : {
			...
			if (bluetooth.interfaces.player === null) {
			...
			const dbus_media = {
				...
				path        : bluetooth.interfaces.player,
				...dbus_base,
			};
...
function interface_objects_root() {
	log.lib(`Getting interface objects for root interface '${bluetooth.interfaces.root}'`);

	// Interfaces added/removed
	bluetooth.service.getInterface(bluetooth.interfaces.root, bluetooth.paths.dbus.obj_man, (error, data) => {
		if (error !== null) {
			log.error(error);
			return false;
		}

		data.on('InterfacesRemoved', (bt_interface, data1) => {
		...
		data.on('InterfacesAdded', bt_interface => {
			...
			if (new RegExp('^/org/bluez/hci[0-9]/dev_[A-F0-9][A-F0-9]_[A-F0-9][A-F0-9]_[A-F0-9][A-F0-9]_[A-F0-9][A-F0-9]_[A-F0-9][A-F0-9]_[A-F0-9][A-F0-9]$').test(bt_interface)) {
				...
				// Update device interface paths
				bluetooth.interfaces.device = bt_interface;
				update.status('bluetooth.interfaces.device', bt_interface);
			...
			if (new RegExp('/player[0-9]$').test(bt_interface)) {
				...
				// Update player interface paths
				bluetooth.interfaces.player = bt_interface;
				update.status('bluetooth.interfaces.player', bt_interface);
...
function interface_properties_adapter() {
	if (bluetooth.interfaces.adapter === null) {
		log.error('Adapter interface is null, cannot get properties');
		return;
	}

	log.lib(`Getting properties for adapter interface '${bluetooth.interfaces.adapter}'`);

	bluetooth.service.getInterface(bluetooth.interfaces.adapter, bluetooth.paths.dbus.prop, (error, data) => {
		if (error !== null) {
		...
		data.on('PropertiesChanged', interface_properties_changed_adapter);
	});
...
function interface_properties_device() {
	...
	log.lib(`Getting properties for device interface '${bluetooth.interfaces.device}'`);

	bluetooth.service.getInterface(bluetooth.interfaces.device, bluetooth.paths.dbus.prop, (error, data) => {
		if (error !== null) {
		...
		data.on('PropertiesChanged', interface_properties_changed_device);
...
function interface_properties_player() {
	...
	log.lib(`Getting properties for player interface '${bluetooth.interfaces.player}'`);

	bluetooth.service.getInterface(bluetooth.interfaces.player, bluetooth.paths.dbus.prop, (error, data) => {
		if (error !== null) {
		...
		data.on('PropertiesChanged', interface_properties_changed_player);
	});
} // interface_properties_player()
...
function interface_properties_changed_adapter(service, data1) {
	...
	log.lib(`Adapter ${bluetooth.interfaces.adapter} service '${service}' property '${propertyKey}' changed`);
...
function interface_properties_changed_device(service, data1) {
	...
	log.lib(`Device ${bluetooth.interfaces.device} service '${service}' property '${propertyKey}' changed'`);
	...
	if (!update.status(`bluetooth.device.${propertyKey}`, value, false)) return;
...
function interface_properties_changed_player(service, data1) {
	...
	if (propertyKey !== 'position') {
		log.lib(`Player ${bluetooth.interfaces.player} service '${service}' property '${propertyKey}' changed`);
	}
	...
	switch (typeof data1[0][1][1][0]) {
		...
		case 'string'  : {
			...
			if (!update.status(`bluetooth.device.${propertyKey}`, value)) return;

			if (config.media.bluetooth.text.mid !== true) return;
...
async function handleBlueZManagedObjects(error, objects) {
	...
	bluetooth.interfaces.device = parse.path;
	update.status('bluetooth.interfaces.device', parse.path);
	...
	interface_properties_device(parse.path);
