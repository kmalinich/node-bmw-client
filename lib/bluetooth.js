...
async function handleInterfaceRoot(error, data) {
	if (error !== null) {
		log.error(error);
		return false;
	}
...
async function interfaceObjectsRoot() {
	log.lib(`Getting root interface '${status.bluetooth.interfaces.root}'`);

	// Interfaces added/removed
	await bluetooth.service.getInterface(status.bluetooth.interfaces.root, bluetooth.paths.dbus.objMan, handleInterfaceRoot);
