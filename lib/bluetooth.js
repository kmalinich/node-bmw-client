module.exports = {
	...
	command : (action) => {
		// Bounce if bluetooth disabled or no device found
	...
	init : (init_cb = null) => {
		if (config.media.bluetooth.enable === false) {
	...
	init_listeners : (init_listeners_cb = null) => {
		// Bounce if bluetooth is disabled or bluetooth event listeners have already been set up
		if (config.media.bluetooth.enable === false || bluetooth.listeners_active === true) {
