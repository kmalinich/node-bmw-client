...
function interfaceObjectsRoot() {
	log.lib(`Getting interface objects for root interface '${status.bluetooth.interfaces.root}'`);

	// Interfaces added/removed
	bluetooth.service.getInterface(status.bluetooth.interfaces.root, bluetooth.paths.dbus.objMan, (error, data) => {
		if (error !== null) {
			log.error(error);
			return false;
		}
...
} // interfaceObjectsRoot()


function interfacePropertiesAdapter() {
...
} // interfacePropertiesAdapter()

function interfacePropertiesDevice() {
	if (status.bluetooth.interfaces.device === null) {
...
} // interfacePropertiesDevice()

function interfacePropertiesPlayer() {
	if (status.bluetooth.interfaces.player === null) {
