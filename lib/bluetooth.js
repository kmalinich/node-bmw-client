...
async function handleBlueZManagedObjects(error, objects) {
	...
	if (bluetooth.device_num > 6) {
		log.error(`Device number ${bluetooth.device_num} is invalid, resetting device number to ${bluetooth.device_num_default}`);
		bluetooth.device_num = bluetooth.device_num_default;
		return;
	}
	...
	power.on('active', async (power_state) => {
		...
		switch (power_state) {
			case false : {
				...
				log.lib(`Device number is currently ${bluetooth.device_num}, resetting device number to ${bluetooth.device_num_default}`);
				bluetooth.device_num = bluetooth.device_num_default;
				break;
			}
