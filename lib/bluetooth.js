...
async function getInterface(interfaceType) {
	...
	bluetooth.service.getInterface(status.bluetooth.interfaces[interfaceType], bluetooth.paths.dbus.prop, async (error, data) => {
...
async function handleInterfacePropertiesChanged(interfaceType, service, propertyData) {
	...
	if (config.media.bluetooth.text.mid === true) {
		if (update.status('mid.text.right', `${status.bluetooth[interfaceType].artist} - ${status.bluetooth[interfaceType].title}`)) {
			MID.refresh_text();
		}
	}
	...
	const strings = {
		artist : status.bluetooth[interfaceType].artist.replace(/feat/gi, 'feat'),
		title  : status.bluetooth[interfaceType].title.replace(/feat/gi, 'feat'),
	};
