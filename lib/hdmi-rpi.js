...
function init_listeners() {
	...
	hdmi_rpi.hdmi_client.on('status', (data) => {
		...
		switch (data.data.type) {
			case 'tv' : {
				update.status('hdmi.rpi.group',       data.data.status.group);
				update.status('hdmi.rpi.mode',        data.data.status.mode);
				update.status('hdmi.rpi.power',       data.data.status.power);
				update.status('hdmi.rpi.progressive', data.data.status.progressive);
				update.status('hdmi.rpi.ratio',       data.data.status.ratio);
				update.status('hdmi.rpi.refreshrate', data.data.status.refreshrate);
				update.status('hdmi.rpi.resolution',  data.data.status.resolution);
				update.status('hdmi.rpi.state',       data.data.status.state);

				break;
			}

			case 'vt' : {
				update.status('hdmi.rpi.tty', data.data.status.tty);
			}
		}
	});
