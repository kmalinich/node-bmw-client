...
function init_listeners() {
	...
	GM.on('open', (data) => {
		...
		if (hdmi_rpi.waiting.ignition === true || hdmi_rpi.waiting.ignition.powerup === true) return;
		...
		switch (data.doors.sealed) {
			...
			case true : {
				...
				if (hdmi_rpi.waiting.open.doors.sealed.true === true) {
		...
		if (hdmi_rpi.waiting.open.doors.sealed.false === false && hdmi_rpi.waiting.open.doors.sealed.true === false) {
			log.msg('Ignition off, doors sealed then unsealed, turning off display');
			hdmi_rpi.waiting.ignition.powerup = true;
			command('poweroff');
		}
	});
	...
	IKE.on('ignition-powerup',  () => {
		hdmi_rpi.waiting.ignition.poweroff = true;
		hdmi_rpi.waiting.ignition.powerup  = false;
	...
	IKE.on('ignition-poweroff', () => {
		...
		hdmi_rpi.waiting.ignition.poweroff = false;
