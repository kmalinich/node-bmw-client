...
module.exports = {
	...
	check_data : (data) => {
        ...
        data.value = 'display text, F/L: '+flags+'/'+layout+', \''+Buffer.from(data.msg).slice(4).toString().replace('�', 'deg').slice(0, -1)+'\'';
				break;

			case 0x24:
				data.command = 'upd';
        data.value   = 'display text, '+Buffer.from(data.msg).slice(2).toString();
				break;
