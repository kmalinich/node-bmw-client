...
function toggle(relay) {
	...
	switch (relay) {
		case 1:
			let new_value = Math.abs(gpio.relay.relay_1.digitalRead()-1);
			gpio.relay.relay_1.writeSync(new_value);
			break;
		case 2:
			let new_value = Math.abs(gpio.relay.relay_2.digitalRead()-1);
...
function set(relay, value) {
	...
	log.msg({
		src : module_name,
		msg : 'Set GPIO '+relay+': '+value,
	});
}

function init(init_callback = null) {
	if (config.gpio.enable !== true || process.arch != 'arm' && cluster.isWorker) {
