...
module.exports = {
	...
	msg : (data) => {
		...
		console.log('       [%s] [%s] %s', center(data.src, 24), center('MESSAGE', 24), data.msg);
	...
	bus : (data) => {
		...
		// Output formatted string
		console.log('[%s] [%s >> %s] [%s]', data.bus, pad(10, data.src.name), pad(data.dst.name, 10), center(data.command, 24), data.value);
