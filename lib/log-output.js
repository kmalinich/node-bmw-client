...
module.exports = {
	...
	bus : (data) => {
		...
		switch (data.bus_orig) {
			...
			default     : data.bus = chalk.green (data.bus);
		}
		...
		data.value = data.value.toString().replace(/true/,  chalk.green('true' ));
		data.value = data.value.toString().replace(/false/, chalk.red(  'false'));
