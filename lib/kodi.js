...
function process_player_data(player) {
	...
	if (player.data) {
		if (player.data.item) {
			if (player.data.item.album) { status.update('kodi.player.album', player.data.item.album); }
			if (player.data.item.type ) { status.update('kodi.player.type',  player.data.item.type);  }
			if (player.data.item.title) { status.update('kodi.player.title', player.data.item.title); }
			if (player.data.item.label) { status.update('kodi.player.title', player.data.item.label); }

			if (player.data.item.artist     ) { status.update('kodi.player.artist', player.data.item.artist[0]);      }
			if (player.data.item.albumartist) { status.update('kodi.player.artist', player.data.item.albumartist[0]); }
		...
		if (player.data.player) {
			if (player.data.player.playerid) { status.update('kodi.player.id', player.data.player.playerid); }
			if (player.data.player.time) {
				if (player.data.player.time.minutes) { status.update('kodi.player.time.minutes', player.data.player.time.minutes); }
				if (player.data.player.time.seconds) { status.update('kodi.player.time.seconds', player.data.player.time.seconds); }
			}
		}
	}
}
...
function get_active_players(get_active_players_callback) {
	...
	kodi.socket.Player.GetActivePlayers().then((players) => {
		if (!players[0]) {
			...
			status.update('kodi.player.id',  players[0].playerid);
			status.update('kodi.player.type',players[0].type);
...
function gui_activate_window(gui_window) {
	...
	return kodi.socket.GUI.ActivateWindow({ 'window' : gui_window }).catch((e) => {
		// Handle errors
		log.msg({
			src : module_name,
...
function input_execute_action(input_action) {
	...
	return kodi.socket.Input.ExecuteAction({ 'action' : input_action }).catch((e) => {
		// Handle errors
		log.msg({
			src : module_name,
...
function command(action) {
	...
	kodi.socket.Player.GetActivePlayers().then((players) => { // List active players
		return Promise.all(players.map((player) => { // Map players
			status.update('kodi.player.id', player.playerid);
