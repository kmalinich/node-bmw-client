...
function autoconfig(autoconfig_callback = null) {
  ...
  kodi_ws(config.media.kodi.host, config.media.kodi.port).then((promise_kodi_socket) => {
    ...
    IKE.text_warning('Kodi:      connected', 1500);
    ...
    kodi.socket.on('error', (error) => {
      ...
      IKE.text_warning('Kodi:   disconnected', 1500);
...
function shutdown(shutdown_callback) {
  if (config.media.kodi.enable !== true) {
...
module.exports = {
  ...
  autoconfig : (autoconfig_callback) => { autoconfig(autoconfig_callback); },
