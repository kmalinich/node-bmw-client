...
function init(init_cb = null) {
	...
	kodi_ws(config.media.kodi.host, config.media.kodi.port).then((promise_kodi_socket) => {
		...
		kodi.socket.notification('AudioLibrary.OnUpdate', (data) => {
			log.msg({ msg : 'Audio library item updated, id : ' + data.data.id + ', type : ' + data.data.type });
		});

		kodi.socket.notification('AudioLibrary.OnRemove', (data) => {
			log.msg({ msg : 'Audio library item removed, id : ' + data.data.id + ', type : ' + data.data.type });
		});
...
IKE.on('ignition-powerup', () => {
	...
	setTimeout(() => {
		if (status.kodi.player.status != 'playing') command('toggle');
	}, 6000);
});
