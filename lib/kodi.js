...
GM.on('open', (data) => {
	...
	if (kodi.waiting.ignition === true || kodi.waiting.ignition.powerup === true) return;

	switch (data.doors.sealed) {
		case false : {
			if (kodi.waiting.open.doors.sealed.false === true) {
				log.msg('No longer waiting for GM.open.doors.sealed.false event');
				kodi.waiting.open.doors.sealed.false = false;
			}
			break;
		}
		...
		case true : {
			...
			if (kodi.waiting.open.doors.sealed.true === true) {
				log.msg('No longer waiting for GM.open.doors.sealed.true event');
				kodi.waiting.open.doors.sealed.true = false;
			}
		}
	}

	// We've successfully waited for all three events
	if (kodi.waiting.open.doors.sealed.false === false && kodi.waiting.open.doors.sealed.true === false) {
		log.msg('Ignition off, doors sealed then unsealed, pausing playback');
		kodi.waiting.ignition.powerup = true;
		command('pause');
	}
});
...
IKE.on('ignition-powerup', () => {
	kodi.waiting.ignition.poweroff = true;
	kodi.waiting.ignition.powerup  = false;
...
IKE.on('ignition-poweroff', () => {
	...
	kodi.waiting.ignition.poweroff = false;
	...
	switch (status.doors.sealed) {
		...
		case true  : {
			...
			kodi.waiting.open.doors.sealed.false = true;
		}
	}

	log.msg('Waiting for GM.open.doors.sealed.true event');
	kodi.waiting.open.doors.sealed.true = true;
