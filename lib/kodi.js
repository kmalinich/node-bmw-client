...
function autoconfig_loop(action = true, callback = null) {
	if (config.media.kodi.enable !== true) {
	...
	switch (action) {
		case true:
			if (kodi.socket === null && status_autoconfig === false) {
				kodi.autoconfig(callback);
...
module.exports = {
	...
	autoconfig_loop : (action, callback) => {
		autoconfig_loop(action, callback);
	},
	...
	notify : (title, message) => {
		...
		if (kodi.socket === null) {
			...
			kodi.autoconfig_loop(true, () => {
				kodi.notify(title, message);
			});
			return;
		}
