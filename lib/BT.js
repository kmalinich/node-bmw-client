module.exports = {
	...
	command : (action) => {
		...
		switch (action) {
			case 'connect':
				...
				TEL.led({ flash_yellow : true, solid_green : true });
				break;
			...
			case 'disconnect':
				...
				TEL.led({ flash_yellow : true, solid_red : true });
				break;

			case 'pause':
				if (status.bluetooth.device.paths.player == null) return;
	...
	init : (init_cb = null) => {
		...
		BT.system_dbus.getObject('org.bluez', '/', (error, objects) => {
			...
			// 4: 3rd paired device
			// ... so on and so forth
			let device_num = config.bluetooth.device_num;
	...
	init_listeners : (init_listeners_cb = null) => {
		...
		IKE.on('ignition-powerup',  () => { BT.command('connect');    });
				...
				switch (typeof data1[0][1][1][0]) {
					...
					default:
						for (let field = 0; field < data1[0][1][1][0].length; field++) {
							...
							// Track information
							update.status('bluetooth.device.media.' + field_name, field_value);
						}

						if (update.status('mid.text_right', status.bluetooth.device.media.title + ' - ' + status.bluetooth.device.media.artist)) {
							MID.refresh_text();
						}
				}
			});
