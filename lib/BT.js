...
module.exports = {
	...
	init : (init_cb = null) => {
		...
		BT.system_dbus.getObject('org.bluez', '/', (error, objects) => {
			...
			objects.as('org.freedesktop.DBus.ObjectManager').GetManagedObjects((error_obj, return_obj) => {
				if (return_obj[2]) {
					let name    = return_obj[2][1][1][1][1][1][1][0];
					let path    = return_obj[2][0];
					let service = return_obj[2][1][1][0];

					// Set variables in status object appropriately
					update.status('bluetooth.devicename',    name);
					update.status('bluetooth.devicepath',    path);
					update.status('bluetooth.deviceservice', service);
	...
	interface_properties_player : (bt_interface) => {
		BT.service.getInterface(bt_interface, 'org.freedesktop.DBus.Properties', (error, data) => {
			...
			update.status('bluetooth.devicepath_player', bt_interface);
