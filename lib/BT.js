				...
				if (BT.listener_active === false) {
					bus.getService('org.bluez').getInterface('/', 'org.freedesktop.DBus.ObjectManager', (err, data) => {
						data.on('InterfacesAdded', (interface, data1, data2) => {
							log.msg({ src: module_name, msg: 'Interface \''+interface+'\' added' });
							console.log(data1);
							console.log(data2);

							if (new RegExp('/player[0-9]$').test(interface)) {
								log.msg({ src: module_name, msg: 'MediaPlayer1 interface \''+interface+'\' added' });

                if (status.bt_device.path_player === null) {
                  bus.getService('org.bluez').getInterface(interface, 'org.freedesktop.DBus.Properties', (err, data) => {
                    data.on('PropertiesChanged', (property, data1, data2) => {
                      log.msg({ src: module_name, msg: 'MediaPlayer1 property \''+property+'\' changed' });
                    });
					...
					bus.getService('org.bluez').getInterface(status.bt_device.path, 'org.freedesktop.DBus.Properties', (err, data) => {
						data.on('PropertiesChanged', (property, data1, data2) => {
							log.msg({ src: module_name, msg: 'Device property \''+property+'\' changed' });
