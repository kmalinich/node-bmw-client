	...
	autoconfig : (callback = null) => {
		...
		bus.getObject('org.bluez', '/', (error, objects) => {
			objects.as('org.freedesktop.DBus.ObjectManager').GetManagedObjects((error_obj, return_obj) => {
				...
				console.log(return_obj[2][1][5][1][0][1][1]);
				...
				log.msg({ src : module_name, msg : 'Configured bluetooth device \''+name+'\', connected: '+connected });
	...
	command : (action) => {
		...
		switch (action) {
			...
			case 'pause':
				...
				bus.invoke({
					path        : status.bt_device.path+'/player2',
					destination : 'org.bluez',
