...
module.exports = {
	...
	init_listeners : (init_listeners_callback = null) => {
		...
		BT.service.getInterface('/', 'org.freedesktop.DBus.ObjectManager', (err, data) => {
			...
			data.on('InterfacesRemoved', (bt_interface) => {
				log.module({ src: module_name, msg: 'Interface \''+bt_interface+'\' removed' });
			});
			...
			data.on('InterfacesAdded', (bt_interface) => {
				...
				if (new RegExp('^/org/bluez/hci[0-9]/dev_[A-F0-9][A-F0-9]_[A-F0-9][A-F0-9]_[A-F0-9][A-F0-9]_[A-F0-9][A-F0-9]_[A-F0-9][A-F0-9]_[A-F0-9][A-F0-9]$').test(bt_interface)) {
					log.module({ src: module_name, msg: 'Device bt_interface \''+bt_interface+'\' added' });
					BT.bt_interface_properties_device(bt_interface);
				}

				// MediaPlayer1 bt_interface
				else if (new RegExp('/player[0-9]$').test(bt_interface)) {
					log.module({ src: module_name, msg: 'MediaPlayer1 bt_interface \''+bt_interface+'\' added' });
					BT.bt_interface_properties_player(bt_interface);
				}
				else {
					// log.module({ src: module_name, msg: 'Interface \''+bt_interface+'\' added' });
				}
	...
	interface_properties_device : (bt_interface) => {
		BT.service.getInterface(bt_interface, 'org.freedesktop.DBus.Properties', (err, data) => {
			data.on('PropertiesChanged', (property, data1) => {
				if (data1[0]) {
					// log.module({ src: module_name, msg: 'Device \''+bt_interface+'\' property \''+data1[0][0].toLowerCase()+'\' changed' });
	...
	interface_properties_player: (bt_interface) => {
		BT.service.getInterface(bt_interface, 'org.freedesktop.DBus.Properties', (err, data) => {
			...
			data.on('PropertiesChanged', (property, data1) => {
				// log.module({ src: module_name, msg: 'MediaPlayer1 \''+bt_interface+'\' property \''+data1[0][0].toLowerCase()+'\' changed' });
