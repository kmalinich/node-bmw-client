...
module.exports = {
  ...
  autoconfig : (autoconfig_callback = null) => {
    if (config.media.bluetooth === false) {
      if (typeof autoconfig_callback === 'function') autoconfig_callback();
      autoconfig_callback = undefined;
      return false;
    }

    if (BT.service === null) BT.service = bus.getService('org.bluez');

    log.module({ src : module_name, msg : 'Performing autoconfig' });
  ...
  autoconfig_listeners : (autoconfig_listeners_callback = null) => {
    if (config.media.bluetooth === false) {
      if (typeof autoconfig_listeners_callback === 'function') autoconfig_listeners_callback();
      autoconfig_listeners_callback = undefined;
      return false;
    }
